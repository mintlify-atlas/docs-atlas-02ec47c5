---
title: "Run Telegram Connector in Development Mode"
...
- **`iter_dialogs()`** — yields 10 fake channels: *Channel 1* through *Channel 10*, each with a fixed date and a username matching the title.
- **`iter_messages()`** — yields an infinite stream of fake messages with continuously decreasing timestamps (starting one minute before `offset_date`, decrementing by 1–16 minutes per message). Each message is assigned to one of the 10 fake channels via a rotating `PeerChannel`.
...
Example response from `GET /channels` in FakeClient mode:
